@extends('layouts.app')
@section('title', 'İşveren Vekili Dashboard')
...
@section('content')
    <div class="px-6 py-6">
        ...
        <div class="flex justify-between items-center mb-8">
            ...
            <span class="px-4 py-2 text-sm bg-indigo-600 text-white rounded-full font-medium">
                İşveren Vekili
            </span>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div class="bg-white shadow rounded-xl border-l-4 border-yellow-500 p-5 flex justify-between items-center">
                <div>
                    <p class="text-xs font-semibold text-yellow-600 uppercase tracking-wider">Final Onay Bekleyen</p>
                    <p class="mt-1 text-2xl font-bold text-gray-800">{{ $stats['pending_approvals'] ?? 0 }}</p>
                </div>
                <i class="fas fa-flag text-gray-300 text-3xl"></i>
            </div>

            <!-- Kapatma Bekleyen -->
            <div class="bg-white shadow rounded-xl border-l-4 border-red-600 p-5 flex justify-between items-center">
                <div>
                    <p class="text-xs font-semibold text-red-600 uppercase tracking-wider">Kapatma Bekleyen</p>
            ...
            <!-- Toplam Onaylanan -->
            <div class="bg-white shadow rounded-xl border-l-4 border-green-600 p-5 flex justify-between items-center">
                <div>
                    <p class="text-xs font-semibold text-green-600 uppercase tracking-wider">Toplam Onaylanan</p>
                    <p class="mt-1 text-2xl font-bold text-gray-800">{{ $stats['total_approved'] ?? 0 }}</p>
                </div>
                <i class="fas fa-check-circle text-gray-300 text-3xl"></i>
            </div>
        </div>

        <!-- Bekleyen Final Onaylar Tablosu -->
        <div class="mt-10 bg-white shadow rounded-xl overflow-hidden">
            <div class="border-b border-gray-200 px-6 py-4 bg-gray-50">
                <h2 class="font-bold text-lg text-indigo-600">Final Onay Bekleyen İş İzinleri</h2>
            </div>
            ...
            <div class="p-6">
                            ...
                            <tbody class="divide-y divide-gray-200">
                                @foreach ($pendingApprovals as $permit)
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        <td class="px-4 py-3">
                                            @if($permit->status === 'pending_employer_approval')
                ...
                @else
                    <div class="text-center py-12">
                        ...
                        <p class="text-gray-500 text-lg">Final onay bekleyen iş izni bulunmamaktadır.</p>
                        <p class="text-gray-400 text-sm mt-2">Tüm izinler onaylanmış veya işlem görmüş durumdadır.</p>
                    </div>
                @endif
            </div>
        </div>
